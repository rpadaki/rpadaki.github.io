...
// Roshan's new code!
// Render abcjs sheet music MIDIs, but only if the <body> has the class
// feature-musicmidi. Do this by removing the <pre><code class="language-abc">
// and replacing it with a <p id="music-X"> which will be used to hold the
// generated sheet music. Remark's code syntax highlighting transforms the
// <code> block into a bunch of
//		<div class="remark-code-line">K: Emin</div>
// one per line, so we have to reassemble those to get back linebreaks.
$(function () {
   if ($("body.feature-musicmidi:not(.feature-nomusicmidi)").length) {
      $("code.language-abc, code.abc").each(function (i, e) {
         var $this = $(this);
         var abc = $this.text();
         if ($this.hasClass("remark-code")) {
         ...
         var p = $('<div id="musicmididummy-' + (i + 1) + '">');
         p.hide();
         $this.parent().hide();
         $this.parent().before(p);
         ABCJS.renderMidi("musicmididummy-" + (i + 1), abc, {
            generateInline: false,
            generateDownload: true
         });
         audioUri = p.find('a').attr('href');
         p.remove();
         $this.parent().before('<a id="musicmidi-' + (i + 1) + '">');
         var a = $('#musicmidi-' + (i + 1)).get(0);
         a.href = audioUri;
         a.innerHTML = "Download MIDI version"
      });
   }
});
